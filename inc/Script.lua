...
KleshaID = KleshaID:gsub("#stast",arg.TheRaws)
...
end,{chat_id_=arg.chat_id_,id_=arg.id_,TheRaws=arg.TheRaws,sender_user_id_=data.id_,msgs=msgs,Namei=Namei,UserNameID=UserNameID,UserNameID1=UserNameID1})


end

end,{chat_id_=msg.chat_id_,id_=msg.id_,TheRaws=msg.TheRaws})
...
redis:set(ws..'liwsGroup'..arg.ChatID,(data.invite_liws_ or ""))
...
.."\n** الايــدي •⊱ { `"..arg.ChatID.."` } ⊰•"
.."\n\nـ •⊱ {  ["..FlterName(GroupName).."]("..(data.invite_liws_ or "")..")  } ⊰•\n"
)
end,{ChatID=msg.chat_id_,MsgID=msg.id_}) 
return false
end
...
sendMsg(arg.ChatID,arg.MsgID,"✦¹  أهلا عزيزي "..arg.TheRawsCmd.." \n** تم تثبيت الرساله \n")
elseif data.ID == "Error" and data.code_ == 6 then
sendMsg(arg.ChatID,arg.MsgID,'✦¹  عذرا لا يمكنني التثبيت .\n✦¹  لست مشرف او لا املك صلاحيه التثبيت \n ')    
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,reply_id=msg.reply_id,TheRawsCmd=msg.TheRawsCmd})
end
return false
end
...
sendMsg(arg.ChatID,arg.MsgID,'✦¹  عذرا عزيزي '..arg.TheRawsCmd..' .\n✦¹  لا توجد رساله مثبته لاقوم بازالتها \n ')    
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,TheRawsCmd=msg.TheRawsCmd})
return false
end
...
return "ঌ بواسطه ⇠ "..msg.TheRawsCmd.."   \n  تم مسح {* "..kerd.." *} من قائمه القرده  \n"
end
...
elseif MsgText[2] and MsgText[2]:match('^%d+$') then 
GetUserID(MsgText[2],action_by_id,{msg=msg,cmd="uwstm"}) 
end 
return false
end
...
if MsgText[1] == "قفل الروابط بالتقييد"  then return tqeed_liws(msg) end
...
if MsgText[1] == "قفل الروابط" 		then return lock_liws(msg) end
...
if MsgText[1] == "فتح الروابط بالتقييد" 	then return fktqeed_liws(msg) 	end
...
if MsgText[1] == "فتح الروابط" 		then return unlock_liws(msg) 	end
...
redis:setex(ws..'WiCmdLiws'..msg.chat_id_..msg.sender_user_id_,500,true)
return '✦¹  حسننا عزيزي  \n✦¹  الان ارسل  رابط مجموعتك '
end

if MsgText[1] == "انشاء رابط" then
if not msg.Creator then return "✦¹  هذا الامر يخص {المطور,المنشئ الاساسي ,المنشئ} فقط  \n" end
if not redis:get(ws..'ExCmdLiws'..msg.chat_id_) then
local LiwsGp = ExportLiws(msg.chat_id_)
if LiwsGp then
LiwsGp = LiwsGp.result
redis:set(ws..'liwsGroup'..msg.chat_id_,LiwsGp)
redis:setex(ws..'ExCmdLiws'..msg.chat_id_,120,true)
...
if not redis:get(ws.."lock_liwsk"..msg.chat_id_) then return "✦¹  الامر معطل من قبل الادارة \n^"  end
if not redis:get(ws..'liwsGroup'..msg.chat_id_) then return "✦¹   لا يوجد رابط\n✦¹ لانشاء رابط ارسل { انشاء رابط } \n" end
local GroupName = redis:get(ws..'group:name'..msg.chat_id_)
local GroupLiws = redis:get(ws..'liwsGroup'..msg.chat_id_)
...
if not msg.Director then return "✦¹  هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end
if not redis:get(ws..'liwsGroup'..msg.chat_id_) then 
return " لا يوجد رابط مضاف اصلا " 
end
redis:del(ws..'liwsGroup'..msg.chat_id_)
...
if MsgText[1] == "تفعيل الرابط" 	then return unlock_liwsk(msg) end 
...
if MsgText[1] == "تعطيل الرابط" 	then return lock_liwsk(msg) end 
...
return "✦¹  أهلا عزيزي "..msg.TheRawsCmd.."  \n✦¹  نورت المجموعه \n" 
end 
end
...
function sendpluse() sendMsg(msg.chat_id_,msg.id_,"✦¹  عذرا لا يمكن تنزيل رتبه مثل رتبتك : "..msg.TheRawsCmd.." \n") end

if riwsuser == 8 then return sendMsg(msg.chat_id_,msg.id_,"ঌ المستخدم  ⇠「 "..NameUser.." 」   \nانه بالتأكيد عضو \n️")  end
huk = false
...
if MsgText[1] == "رتبتي" then return '✦¹  رتبتك ⇠ '..msg.TheRaws..'\n' end
...
ws.."WiCmdLiws"..msg.chat_id_..msg.sender_user_id_,
...
text = text:gsub("#stast",msg.TheRaws)
...
if msg.Director and (msg.text:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.text:match("[Hh][Tt][Tt][Pp]://") or msg.text:match("[Ww][Ww][Ww].") or msg.text:match(".[Cc][Oo][Mm]") or msg.text:match(".[Oo][Rr][Gg]")) and redis:get(ws.."WiCmdLiws"..msg.chat_id_..msg.sender_user_id_) then 
redis:del(ws.."WiCmdLiws"..msg.chat_id_..msg.sender_user_id_)
redis:set(ws..'liwsGroup'..msg.chat_id_,msg.text)
...
local function get_liws(target)
  local data = load_data(_config.moderation.data)
  local group_liws = data[tostring(target)]['settings']['set_liws']
  if not group_liws or group_liws == nil then 
    return "No liws"
  end
  return "Group liws:\n"..group_liws
end
...
if (msg.content_.ID == "MessageChatJoinByLiws") then
if redis:get(ws..'welcome:get'..msg.chat_id_) then
GetUserID(msg.sender_user_id_,function(arg,data)
welcome = (redis:get(ws..'welcome:msg'..msg.chat_id_) or "ঌ مرحباً عزيزي\nঌ نورت المجموعة \n")
...
local welcome = welcome:gsub("#stast",msg.TheRaws)
local welcome = welcome:gsub("#game",Get_Ttl(msgs))
local welcome = welcome:gsub("#msgs",msgs)
local welcome = welcome:gsub("#auto",points)
local welcome = welcome:gsub("#edit",edited)
...
or msg.text:match("[Ww][Ww][Ww].") 
or msg.text:match(".[Cc][Oo][Mm]")) 
and redis:get(ws..':tqeed_liws:'..msg.chat_id_)  then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
print("\27[1;31m The user i restricted becuse send liws \27[0m")
if data.ID == "Error" and data.code_ == 6 then
return sendMsg(msg.chat_id_,msg.id_,'✦¹  لا يمكنني مسح الرساله المخالفه .\n✦¹  لست مشرف او ليس لدي صلاحيه  الحذف \n ')    
end
Restrict(msg.chat_id_,msg.sender_user_id_,1)
...
and redis:get(ws..'lock_liws'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
print("\27[1;31m Msg Del becuse send liws \27[0m")
if data.ID == "Error" and data.code_ == 6 then
return sendMsg(msg.chat_id_,msg.id_,'✦¹  لا يمكنني مسح الرساله المخالفه .\n✦¹  لست مشرف او ليس لدي صلاحيه  الحذف \n ')    
end
if redis:get(ws..'lock_woring'..msg.chat_id_) then
...
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
print("\27[1;31m Msg Del becuse send web liws \27[0m")
if data.ID == "Error" and data.code_ == 6 then
return sendMsg(msg.chat_id_,msg.id_,'✦¹  لا يمكنني مسح الرساله المخالفه .\n✦¹  لست مشرف او ليس لدي صلاحيه  الحذف \n ')    
end
if redis:get(ws..'lock_woring'..msg.chat_id_) then
...
and redis:get(ws..'lock_liws'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
print("\27[1;31m Msg Del becuse send liws caption \27[0m")
if data.ID == "Error" and data.code_ == 6 then
return sendMsg(msg.chat_id_,msg.id_,'✦¹  لا يمكنني مسح الرساله المخالفه .\n✦¹  لست مشرف او ليس لدي صلاحيه  الحذف \n ')    
end
if redis:get(ws..'lock_woring'..msg.chat_id_) then
