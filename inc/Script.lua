...
local function iws(msg,MsgText)
    ...
    return '👤¦  عدد جهاتك »【'..(redis:get(ws..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..'】 . \n🐾'
    end
    ...
    if UserID == 1399282735 or UserID == 656329268 then return sendMsg(msg.chat_id_,msg.id_,"📛*¦* لآ يمكنك تنفيذ الامر ضد مطور السورس \n❕") end
    ...
    if not redis:get(ws..":TEXT_SUDO") then
    return '⚙️¦ اوه 🙀 لا يوجد كليشه مطور اصلا ☹️\n🛠' end
    redis:del(ws..':TEXT_SUDO') 
    ...
    return'🚸*¦* شـرط آلتفعيل آلبوت آذآ گآنت آلمـجمـوعهہ‏‏ آگثر مـن *【'..redis:get(ws..':addnumberusers')..'】* عضـو  🍁\n' 
    end 
    end
    ...
    echo '*------------------------------\n*📡 •⊱ { Location } ⊰•\n*»» '`curl http://th3ws.com/ip/location`'*'
    echo '*------------------------------\n*👨🏾‍🔧 •⊱ { Server[_]Login } ⊰•\n*»» '`whoami`'*'
    echo '*------------------------------\n*🔌 •⊱ { Uptime } ⊰•  \n*»» '"$uptime"'*'
    ]]):read('*all')
    end
