...
local function iLANA(msg,MsgText)
    ...
    if not redis:get(LANA..'group:add'..msg.chat_id_) then return sendMsg(msg.chat_id_,msg.id_,'📬¦ المجموعه بالتأكيد ✓️ تم تعطيلها \n📮¦ بواسطه ⋙「 '..NameUser..' 」 \n') end  
    ...
    KleshaID = KleshaID:gsub("{البوت}",redis:get(LANA..':NameBot:'))
    ...
    return sendMsg(msg.chat_id_,msg.id_,"المالك :\n["..v.user.first_name.."](t.me/"..(v.user.username or "wViiVw"))
    end
    end
    ...
    if not redis:get(LANA..'linkGroup'..msg.chat_id_) then return "📡*¦* اوه 🙀 لا يوجد رابط ☹️\n🔖*¦*لانشاء رابط ارسل { انشاء رابط } \n📡" end
    ...
    if not msg.Admin then return "📪¦ هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n" end
    if not redis:get(LANA.."lock_takkl"..msg.chat_id_) then  return "📛*¦* الامر معطل من قبل الادراة" end 
    if redis:get(LANA.."chat:tagall"..msg.chat_id_) then  return "📛*¦* يمكنك عمل تاك للكل كل *5 دقائق* فقط" end 
    ...
    return '👤¦  عدد جهاتك »【'..(redis:get(LANA..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..'】 . \n🐾'
    end
    ...
    sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(LANA..':NameBot:')..[[ 🎖
    ...
    if not redis:get(LANA..":TEXT_SUDO") then
    return '⚙️¦ اوه 🙀 لا يوجد كليشه مطور اصلا ☹️\n🛠' end
    redis:del(LANA..':TEXT_SUDO') 
    ...
    return'🚸*¦* شـرط آلتفعيل آلبوت آذآ گآنت آلمـجمـوعهہ‏‏ آگثر مـن *【'..redis:get(LANA..':addnumberusers')..'】* عضـو  🍁\n' 
    end 
    end
    
    if MsgText[1] == 'المجموعات' or MsgText[1] == "المجموعات 🔝" then 
    if not msg.SudoUser then return "📪¦ هذا الامر يخص {المطور} فقط  \n" end
    return '📮*¦* عدد المجموعات المفعلة » `'..redis:scard(LANA..'group:ids')..'`  ➼' 
    end
    ...
    return send_msg(msg.chat_id_,redis:get(LANA..":TEXT_SUDO") or SUDO_NAME,msg.id_)
    end,nil)
    end
...
return false
    ...
    local Get_Files, res = https.request("https://raw.githubusercontent.com/wViiVw/wViiVw.github.io/master/plugins/"..FileName)
    if res == 200 then
    ...
    text = text:gsub("{البوت}",redis:get(LANA..':NameBot:'))
    ...
    welcome = (redis:get(LANA..'welcome:msg'..msg.chat_id_) or "🔖¦ مرحباً عزيزي\n🔖¦ نورت المجموعة \n💂🏼‍♀️")
    welcome = welcome:gsub("{القوانين}", redis:get(LANA..'rulse:msg'..msg.chat_id_) or "🔖¦ مرحبأ عزيري 👋🏻 القوانين كلاتي 👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه \n🔖¦ ممنوع التكلم بلطائفه \n??¦ الرجاء احترام المدراء والادمنيه 😅\n")
    ...
    local welcome = welcome:gsub("{البوت}",redis:get(LANA..':NameBot:'))
    ...
    GetUserID(msg.sender_user_id_,function(arg,data)
    welcome = (redis:get(LANA..'welcome:msg'..msg.chat_id_) or "🔖¦ مرحباً عزيزي\n🔖¦ نورت المجموعة \n💂🏼‍♀️")
    welcome = welcome:gsub("{القوانين}", redis:get(LANA..'rulse:msg'..msg.chat_id_) or "🔖¦ مرحبأ عزيري 👋🏻 القوانين  👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه \n🔖¦ ممنوع التكلم بلطائفه \n🔖¦ الرجاء احترام المدراء والادمنيه 😅\n")
    ...
    rem_data_group(msg.chat_id_)
    sendMsg(SUDO_ID,0,'🕵🏼️‍♀️¦ انتهى الاشتراك في احد المجموعات ✋🏿\n👨🏾‍🔧¦ المجموعه : '..FlterName(redis:get(LANA..'group:name'..msg.chat_id_))..'🍃\n💂🏻‍♀️¦ ايدي : '..msg.chat_id_)
